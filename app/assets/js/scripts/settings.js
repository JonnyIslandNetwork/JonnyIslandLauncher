...
ipcRenderer.on(MSFT_OPCODE.REPLY_LOGIN, (_, ...arguments_) => {
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        ...
        document.getElementById('waitingTextContainer').innerText = 'Retrieving your account information from Microsoft'
        ...
        if (Object.prototype.hasOwnProperty.call(queryMap, 'error')) {
            ...
            AuthManager.addMicrosoftAccount(authCode).then(value => {
                ...
                switchView(getCurrentView(), viewOnClose, 500, 500, async () => {
                    if (viewOnClose != VIEWS.settings)document.getElementById('frameBar').style.backgroundColor = 'transparent'
...
function processLogOut(val, isLastAccount) {
    ...
    if (targetAcc.type === 'microsoft') {
        ...
        switchView(getCurrentView(), VIEWS.waiting, 500, 500, () => {
            document.getElementById('waitingTextContainer').innerText = 'Removing your Microsoft account from the launcher'
...
function populateReleaseNotes() {
    $.ajax({
        url: 'https://github.com/jonnyislandnetwork/jonnyislandlauncher/releases.atom',
        success: (data) => {
